...
public class SwcWarrNotifyClass extends Thread {
    ...
    public SwcWarrNotifyClass(String sourcePath, SwcWarrService swcWarrService) {
    ...
    class MyJnotifyListner implements JNotifyListener {
        ...
        public void fileCreated(int i, String rootPath, String name) {
            ...
            String tmpFile = "tmp";
            // 进入方法之前必须判断文件是否有效(不需要判断文件长度，因为后面将判断文件是否写入完成)
        ...
        public void run() {
            ...
            while (true) {
                ...
                if (!StringUtils.isEmpty(filePath)) {
                    int size = swcWarrService.insertSwcWarrBase(new File(filePath));
                    logger.info(TypeEnum.TYPE_SWC_WARR.getName() + " onFileCreated: " + size + " 条数据");
                    try {
                        Thread.sleep(100);
                    } catch (InterruptedException e) {
                        e.printStackTrace();
