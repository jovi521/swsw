 ...
 */
@Component
public class RadarPhasedArrayListener implements FileAlterationListener {
     ...
     */
    @Autowired
    public RadarPhasedArrayListener(RadarPhasedArrayService radarPhasedArrayService, CommonPath commonPath) {
    ...
    public void onFileCreate(File fileSource) {
        logger.info(TypeEnum.TYPE_RADAR_PHASED_ARRAY.getName() + " onFileCreate: " + fileSource);
        String tmpFile = "tmp";
        int size = 0;
        // 进入方法之前必须判断文件是否有效
        if (fileSource.length() != 0 && !fileSource.getName().contains(tmpFile)) {
            if (fileSource.getPath().contains(commonPath.getBaseMonitorPath().replace("/", ""))) {
                size = radarPhasedArrayService.insertRadarPhasedArrayBase(fileSource);
            } else if (fileSource.getPath().contains(commonPath.getParseMonitorPath().replace("/", ""))) {
                size = radarPhasedArrayService.insertRadarPhasedArrayParse(fileSource);
            }
            logger.info(TypeEnum.TYPE_RADAR_PHASED_ARRAY.getName() + " onFileCreated: " + size + " 条数据");
        } else {
            logger.info(TypeEnum.TYPE_RADAR_PHASED_ARRAY.getName() + " 新增无效文件：" + fileSource.getName() + "。请检查数据源是否正确");
        }
    }
