package com.cdyw.swsw.data.common.notify.ecmwf;
...
public class EcmwfHrNotifyClass extends Thread {
    ...
    public EcmwfHrNotifyClass(String sourcePath, EcmwfHrService ecmwfHrService) {
        ...
        public void run() {
            ...
            while (true) {
                ...
                if (!StringUtils.isEmpty(filePath)) {
                    int size = ecmwfHrService.insertEcmwfHrBase(new File(filePath));
                    logger.info(TypeEnum.TYPE_ECMWF_HR.getName() + " onFileCreated: " + size + " 条数据");
                    try {
                        Thread.sleep(100);
                    } catch (InterruptedException e) {
                        e.printStackTrace();
