 ...
 */
@Component
public class RadarWeatherListener implements FileAlterationListener {
     ...
     */
    @Autowired
    public RadarWeatherListener(RadarWeatherService radarWeatherService, CommonPath commonPath) {
    ...
    public void onFileCreate(File fileSource) {
        ...
        if (fileSource.length() != 0 && !fileSource.getName().contains(tmpFile)) {
            if (fileSource.getPath().contains(commonPath.getBaseMonitorPath().replace("/", ""))) {
                size = radarWeatherService.insertRadarWeatherBase(fileSource);
            } else if (fileSource.getPath().contains(commonPath.getParseMonitorPath().replace("/", ""))) {
                size = radarWeatherService.insertRadarWeatherParse(fileSource);
            }
