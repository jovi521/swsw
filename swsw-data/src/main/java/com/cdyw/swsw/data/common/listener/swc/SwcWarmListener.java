 ...
 */
@Component
public class SwcWarmListener implements FileAlterationListener {
    ...
    @Autowired
    public SwcWarmListener(SwcWarmService swcWarmService) {
    ...
    public void onFileCreate(File fileSource) {
        ...
        if (fileSource.length() != 0 && !fileSource.getName().contains(tmpFile)) {
            size = swcWarmService.insertSwcWarmBase(fileSource);
